...
- **Explain Code (Etrans)**: Uses Gemini AI to both translate AND explain code/text in detail.
...
2. **Commands**:
   ...
   - `:Etrans [target_lang]`: Translate AND Explain using Gemini AI (Requires API Key).
     - Example: `:Etrans vi` (translate and explain in Vietnamese).
...
vim.keymap.set("v", "<leader>Ge", ":'<,'>Etrans<CR>", { desc = "Gemini Explain" })
```
